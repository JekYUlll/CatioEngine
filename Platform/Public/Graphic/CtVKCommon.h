    ...
    *outEnableCount = 0;
    ...
    for(int i = 0; i < requestedCount; i++) {
        ...
        for(int j = 0; j < availableCount; j++) {
            const char *availableName = bExtension ? static_cast<VkExtensionProperties *>(available)[j].extensionName : static_cast<VkLayerProperties *>(available)[j].layerName;
            if(std::strcmp(availableName, requestFeatures[i].name) == 0) {
